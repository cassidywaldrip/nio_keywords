...
| `real_estate.csv` | 575 rows, 3 columns (`keyword,category,tier`). 3 tiers for filtering: core, related, and contextual. Also has several categories that can be used for filtering. Generated by Claude with the following prompt: "Please generate a list of words related to real estate. This list will be used to identify text, news, and online posts that have to do with real estate." Verified and added to by Cassidy. |
...
| `glp1-keywords.csv` | 79 rows, 3 columns (`keyword,category,tier`). 3 tiers for filtering: core, related, and contextual. Also has several categories that can be used for filtering. Generated by Claude with the following prompt: "Please generate a list of words related to GLP-1s. This list will be used to identify text, news, and online posts that have to do with GLP-1s." Verified and added to by Cassidy. |
...
**3,467 core emotion-related keywords** (filtered) and **13,041 full keywords** (unfiltered), compiled from NRC EmoLex, AFINN-111, VADER, and manually curated vocabulary. The core set uses cross-lexicon validation — a word is retained only if at least 2 independent lexicons agree it is emotion-related, or if it belongs to the manually curated set — removing noisy entries (e.g., "anarchism", "analyst") while preserving genuine emotion words. See [`emotional_states/README.md`](emotional_states/README.md) for full details and citations.
